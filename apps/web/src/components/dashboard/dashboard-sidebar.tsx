"use client";

import React, { useState } from 'react';
import Link from 'next/link';
import {
  ChevronLeft,
  ChevronRight,
  Plus,
  Check,
  Trash2
} from 'lucide-react';
import { cn } from '@/lib/utils';
import { Button } from '@/components/ui/button';
import {
  Dialog,
  DialogContent,
  DialogDescription,
  DialogHeader,
  DialogTitle,
  DialogTrigger,
} from '@/components/ui/dialog';

// 社交媒体图标组件
const TwitterIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
  </svg>
);

const FacebookIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/>
  </svg>
);

const InstagramIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
  </svg>
);

const LinkedInIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
  </svg>
);

const YouTubeIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M23.498 6.186a3.016 3.016 0 0 0-2.122-2.136C19.505 3.545 12 3.545 12 3.545s-7.505 0-9.377.505A3.017 3.017 0 0 0 .502 6.186C0 8.07 0 12 0 12s0 3.93.502 5.814a3.016 3.016 0 0 0 2.122 2.136c1.871.505 9.376.505 9.376.505s7.505 0 9.377-.505a3.015 3.015 0 0 0 2.122-2.136C24 15.93 24 12 24 12s0-3.93-.502-5.814zM9.545 15.568V8.432L15.818 12l-6.273 3.568z"/>
  </svg>
);

const TikTokIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z"/>
  </svg>
);

const MediumIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M13.54 12a6.8 6.8 0 01-6.77 6.82A6.8 6.8 0 010 12a6.8 6.8 0 016.77-6.82A6.8 6.8 0 0113.54 12zM20.96 12c0 3.54-1.51 6.42-3.38 6.42-1.87 0-3.39-2.88-3.39-6.42s1.52-6.42 3.39-6.42 3.38 2.88 3.38 6.42M24 12c0 3.17-.53 5.75-1.19 5.75-.66 0-1.19-2.58-1.19-5.75s.53-5.75 1.19-5.75C23.47 6.25 24 8.83 24 12z"/>
  </svg>
);

const PinterestIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M12.017 0C5.396 0 .029 5.367.029 11.987c0 5.079 3.158 9.417 7.618 11.162-.105-.949-.199-2.403.041-3.439.219-.937 1.406-5.957 1.406-5.957s-.359-.72-.359-1.781c0-1.663.967-2.911 2.168-2.911 1.024 0 1.518.769 1.518 1.688 0 1.029-.653 2.567-.992 3.992-.285 1.193.6 2.165 1.775 2.165 2.128 0 3.768-2.245 3.768-5.487 0-2.861-2.063-4.869-5.008-4.869-3.41 0-5.409 2.562-5.409 5.199 0 1.033.394 2.143.889 2.741.099.12.112.225.085.345-.09.375-.293 1.199-.334 1.363-.053.225-.172.271-.401.165-1.495-.69-2.433-2.878-2.433-4.646 0-3.776 2.748-7.252 7.92-7.252 4.158 0 7.392 2.967 7.392 6.923 0 4.135-2.607 7.462-6.233 7.462-1.214 0-2.354-.629-2.758-1.379l-.749 2.848c-.269 1.045-1.004 2.352-1.498 3.146 1.123.345 2.306.535 3.55.535 6.607 0 11.985-5.365 11.985-11.987C23.97 5.39 18.592.026 11.985.026L12.017 0z"/>
  </svg>
);

const RedditIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M12 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0zm5.01 4.744c.688 0 1.25.561 1.25 1.249a1.25 1.25 0 0 1-2.498.056l-2.597-.547-.8 3.747c1.824.07 3.48.632 4.674 1.488.308-.309.73-.491 1.207-.491.968 0 1.754.786 1.754 1.754 0 .716-.435 1.333-1.01 1.614a3.111 3.111 0 0 1 .042.52c0 2.694-3.13 4.87-7.004 4.87-3.874 0-7.004-2.176-7.004-4.87 0-.183.015-.366.043-.534A1.748 1.748 0 0 1 4.028 12c0-.968.786-1.754 1.754-1.754.463 0 .898.196 1.207.49 1.207-.883 2.878-1.43 4.744-1.487l.885-4.182a.342.342 0 0 1 .14-.197.35.35 0 0 1 .238-.042l2.906.617a1.214 1.214 0 0 1 1.108-.701zM9.25 12C8.561 12 8 12.562 8 13.25c0 .687.561 1.248 1.25 1.248.687 0 1.248-.561 1.248-1.249 0-.688-.561-1.249-1.249-1.249zm5.5 0c-.687 0-1.248.561-1.248 1.25 0 .687.561 1.248 1.249 1.248.688 0 1.249-.561 1.249-1.249 0-.687-.562-1.249-1.25-1.249zm-5.466 3.99a.327.327 0 0 0-.231.094.33.33 0 0 0 0 .463c.842.842 2.484.913 2.961.913.477 0 2.105-.056 2.961-.913a.361.361 0 0 0 .029-.463.33.33 0 0 0-.464 0c-.547.533-1.684.73-2.512.73-.828 0-1.979-.196-2.512-.73a.326.326 0 0 0-.232-.095z"/>
  </svg>
);

const DiscordIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M20.317 4.492c-1.53-.69-3.17-1.2-4.885-1.49a.075.075 0 0 0-.079.036c-.21.369-.444.85-.608 1.23a18.566 18.566 0 0 0-5.487 0 12.36 12.36 0 0 0-.617-1.23A.077.077 0 0 0 8.562 3c-1.714.29-3.354.8-4.885 1.491a.07.07 0 0 0-.032.027C.533 9.093-.32 13.555.099 17.961a.08.08 0 0 0 .031.055 20.03 20.03 0 0 0 5.993 2.98.078.078 0 0 0 .084-.026c.462-.62.874-1.275 1.226-1.963.021-.04.001-.088-.041-.104a13.201 13.201 0 0 1-1.872-.878.075.075 0 0 1-.008-.125c.126-.093.252-.19.372-.287a.075.075 0 0 1 .078-.01c3.927 1.764 8.18 1.764 12.061 0a.075.075 0 0 1 .079.009c.12.098.245.195.372.288a.075.075 0 0 1-.006.125c-.598.344-1.22.635-1.873.877a.075.075 0 0 0-.041.105c.36.687.772 1.341 1.225 1.962a.077.077 0 0 0 .084.028 19.963 19.963 0 0 0 6.002-2.981.076.076 0 0 0 .032-.054c.5-5.094-.838-9.52-3.549-13.442a.06.06 0 0 0-.031-.028zM8.02 15.278c-1.182 0-2.157-1.069-2.157-2.38 0-1.312.956-2.38 2.157-2.38 1.21 0 2.176 1.077 2.157 2.38 0 1.312-.956 2.38-2.157 2.38zm7.975 0c-1.183 0-2.157-1.069-2.157-2.38 0-1.312.955-2.38 2.157-2.38 1.21 0 2.176 1.077 2.157 2.38 0 1.312-.946 2.38-2.157 2.38z"/>
  </svg>
);

const TelegramIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M11.944 0A12 12 0 0 0 0 12a12 12 0 0 0 12 12 12 12 0 0 0 12-12A12 12 0 0 0 12 0a12 12 0 0 0-.056 0zm4.962 7.224c.1-.002.321.023.465.14a.506.506 0 0 1 .171.325c.016.093.036.306.02.472-.18 1.898-.962 6.502-1.36 8.627-.168.9-.499 1.201-.82 1.23-.696.065-1.225-.46-1.9-.902-1.056-.693-1.653-1.124-2.678-1.8-1.185-.78-.417-1.21.258-1.91.177-.184 3.247-2.977 3.307-3.23.007-.032.014-.15-.056-.212s-.174-.041-.249-.024c-.106.024-1.793 1.14-5.061 3.345-.48.33-.913.49-1.302.48-.428-.008-1.252-.241-1.865-.44-.752-.245-1.349-.374-1.297-.789.027-.216.325-.437.893-.663 3.498-1.524 5.83-2.529 6.998-3.014 3.332-1.386 4.025-1.627 4.476-1.635z"/>
  </svg>
);

const ThreadsIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M12.186 24h-.007c-3.581-.024-6.334-1.205-8.184-3.509C2.35 18.44 1.5 15.586 1.472 12.01v-.017c.03-3.579.879-6.43 2.525-8.482C5.845 1.205 8.6.024 12.18 0h.014c2.746.02 5.043.725 6.826 2.098 1.677 1.29 2.858 3.13 3.509 5.467l-2.04.569c-1.104-3.96-3.898-5.984-8.304-6.015-2.91.022-5.11.936-6.54 2.717C4.307 6.504 3.616 8.914 3.589 12c.027 3.086.718 5.496 2.057 7.164 1.43 1.781 3.631 2.695 6.54 2.717 2.623-.02 4.358-.631 5.8-2.045 1.647-1.613 1.618-3.593 1.09-4.798-.31-.71-.873-1.3-1.634-1.75-.192 1.352-.622 2.446-1.284 3.272-.886 1.102-2.14 1.704-3.73 1.79-1.202.065-2.361-.218-3.259-.801-1.063-.689-1.685-1.74-1.752-2.964-.065-1.19.408-2.285 1.33-3.082.88-.76 2.119-1.207 3.583-1.291a13.853 13.853 0 0 1 3.02.142l-.126.742a12.954 12.954 0 0 0-2.894-.134c-1.242.072-2.237.44-2.963 1.095-.684.617-1.024 1.435-.956 2.301.05.805.483 1.511 1.216 1.991.645.423 1.504.607 2.42.517 1.17-.116 2.144-.589 2.896-1.408 1.031-1.12 1.302-2.812 1.015-5.044a.764.764 0 0 1 .08-.323c1.852.545 3.137 1.564 3.82 3.027.767 1.642.794 4.063-1.294 6.073-1.843 1.783-4.034 2.606-7.304 2.606z"/>
  </svg>
);

const BlueskyIcon = ({ className, style }: { className?: string; style?: React.CSSProperties }) => (
  <svg viewBox="0 0 24 24" className={className} fill="currentColor" style={style}>
    <path d="M12 10.8c-1.087-2.114-4.046-6.053-6.798-7.995C2.566.944 1.561 1.266.902 1.565.139 1.908 0 3.08 0 3.768c0 .69.378 5.65.624 6.479.815 2.736 3.713 3.66 6.383 3.364.136-.02.275-.039.415-.056-.138.022-.276.04-.415.056-3.912.58-7.387 2.005-2.83 7.078 5.013 5.19 6.87-1.113 7.823-4.308.953 3.195 2.05 9.271 7.733 4.308 4.267-4.308 1.172-6.498-2.74-7.078a8.741 8.741 0 0 1-.415-.056c.14.017.279.036.415.056 2.67.297 5.568-.628 6.383-3.364.246-.828.624-5.79.624-6.478 0-.69-.139-1.861-.902-2.206-.659-.298-1.664-.62-4.3 1.24C16.046 4.748 13.087 8.687 12 10.8z"/>
  </svg>
);

interface SocialPlatformDefinition {
  id: string;
  platform: string;
  icon: React.ComponentType<{ className?: string; style?: React.CSSProperties }>;
  color: string;
  category: 'social' | 'professional' | 'media' | 'messaging';
}

interface SocialAccount extends SocialPlatformDefinition {
  username: string;
  connected: boolean;
  isVisible: boolean;
}

// 所有可用的社交媒体平台定义
const ALL_PLATFORMS: SocialPlatformDefinition[] = [
  {
    id: 'twitter',
    platform: 'Twitter',
    icon: TwitterIcon,
    color: '#1da1f2',
    category: 'social'
  },
  {
    id: 'facebook',
    platform: 'Facebook',
    icon: FacebookIcon,
    color: '#1877f2',
    category: 'social'
  },
  {
    id: 'instagram',
    platform: 'Instagram',
    icon: InstagramIcon,
    color: '#e4405f',
    category: 'social'
  },
  {
    id: 'tiktok',
    platform: 'TikTok',
    icon: TikTokIcon,
    color: '#000000',
    category: 'social'
  },
  {
    id: 'threads',
    platform: 'Threads',
    icon: ThreadsIcon,
    color: '#000000',
    category: 'social'
  },
  {
    id: 'bluesky',
    platform: 'Bluesky',
    icon: BlueskyIcon,
    color: '#0085ff',
    category: 'social'
  },
  {
    id: 'linkedin',
    platform: 'LinkedIn',
    icon: LinkedInIcon,
    color: '#0077b5',
    category: 'professional'
  },
  {
    id: 'youtube',
    platform: 'YouTube',
    icon: YouTubeIcon,
    color: '#ff0000',
    category: 'media'
  },
  {
    id: 'medium',
    platform: 'Medium',
    icon: MediumIcon,
    color: '#000000',
    category: 'media'
  },
  {
    id: 'pinterest',
    platform: 'Pinterest',
    icon: PinterestIcon,
    color: '#e60023',
    category: 'media'
  },
  {
    id: 'reddit',
    platform: 'Reddit',
    icon: RedditIcon,
    color: '#ff4500',
    category: 'social'
  },
  {
    id: 'discord',
    platform: 'Discord',
    icon: DiscordIcon,
    color: '#5865f2',
    category: 'messaging'
  },
  {
    id: 'telegram',
    platform: 'Telegram',
    icon: TelegramIcon,
    color: '#0088cc',
    category: 'messaging'
  }
];

// 默认显示在侧边栏的平台
const DEFAULT_VISIBLE_PLATFORMS = ['twitter', 'facebook', 'instagram', 'linkedin', 'youtube'];

interface DashboardSidebarProps {
  isCollapsed: boolean;
  onToggleCollapse: (collapsed: boolean) => void;
}

export const DashboardSidebar: React.FC<DashboardSidebarProps> = ({
  isCollapsed,
  onToggleCollapse
}) => {
  const [isDialogOpen, setIsDialogOpen] = useState(false);

  // 初始化社交账户数据（模拟数据，实际应从数据库获取）
  const [socialAccounts, setSocialAccounts] = useState<SocialAccount[]>(() => {
    return ALL_PLATFORMS.map(platform => ({
      ...platform,
      username: platform.id === 'twitter' || platform.id === 'instagram' ? '@username' : 'Not connected',
      connected: platform.id === 'twitter' || platform.id === 'instagram',
      isVisible: DEFAULT_VISIBLE_PLATFORMS.includes(platform.id)
    }));
  });

  // 获取当前可见的账户
  const visibleAccounts = socialAccounts.filter(account => account.isVisible);

  // 按分类分组的所有平台
  const platformsByCategory = {
    social: ALL_PLATFORMS.filter(p => p.category === 'social'),
    professional: ALL_PLATFORMS.filter(p => p.category === 'professional'),
    media: ALL_PLATFORMS.filter(p => p.category === 'media'),
    messaging: ALL_PLATFORMS.filter(p => p.category === 'messaging'),
  };

  const handleConnectPlatform = (platformId: string) => {
    console.log('Connecting to:', platformId);
    // TODO: Implement OAuth connection flow
  };

  const handleTogglePlatformVisibility = (platformId: string) => {
    setSocialAccounts(prev =>
      prev.map(account =>
        account.id === platformId
          ? { ...account, isVisible: !account.isVisible }
          : account
      )
    );
  };

  const handleRemovePlatform = (platformId: string) => {
    setSocialAccounts(prev =>
      prev.map(account =>
        account.id === platformId
          ? { ...account, isVisible: false }
          : account
      )
    );
  };

  return (
    <aside
      className={cn(
        "hidden md:fixed left-0 top-16 h-[calc(100vh-4rem)] bg-card border-r border-border transition-all duration-300 z-30",
        isCollapsed ? "md:block md:w-20" : "md:block md:w-72"
      )}
    >
      {/* Toggle Button */}
      <button
        onClick={() => onToggleCollapse(!isCollapsed)}
        className="absolute -right-3 top-6 w-6 h-6 rounded-full bg-card border border-border flex items-center justify-center hover:bg-accent transition-colors"
      >
        {isCollapsed ? (
          <ChevronRight className="w-4 h-4" />
        ) : (
          <ChevronLeft className="w-4 h-4" />
        )}
      </button>

      <div className="flex flex-col h-full overflow-y-auto">
        {/* Social Media Accounts Section */}
        <div className="flex-1 px-3 py-3">
          <div className={cn(
            "flex items-center justify-between mb-3",
            isCollapsed && "justify-center"
          )}>
            {!isCollapsed && (
              <h3 className="text-sm font-semibold text-foreground px-3">
                Social Accounts
              </h3>
            )}
          </div>

          <div className="space-y-2">
            {visibleAccounts.map((account) => {
              const Icon = account.icon;

              return (
                <div
                  key={account.id}
                  className={cn(
                    "rounded-lg border transition-all hover:shadow-sm group",
                    account.connected
                      ? "bg-card border-border"
                      : "bg-muted/30 border-dashed border-muted-foreground/30"
                  )}
                >
                  {isCollapsed ? (
                    // Collapsed view - just icon
                    <button
                      onClick={() => !account.connected && handleConnectPlatform(account.id)}
                      className="w-full p-3 flex items-center justify-center"
                      title={account.platform}
                    >
                      <div className="relative">
                        <Icon
                          className="w-5 h-5"
                          style={{ color: account.connected ? account.color : undefined }}
                        />
                        {account.connected && (
                          <div className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 rounded-full border-2 border-card" />
                        )}
                      </div>
                    </button>
                  ) : (
                    // Expanded view - full card
                    <div className="p-3">
                      <div className="flex items-center justify-between mb-2">
                        <div className="flex items-center gap-2">
                          <Icon
                            className="w-5 h-5"
                            style={{ color: account.color }}
                          />
                          <span className="font-medium text-sm">
                            {account.platform}
                          </span>
                        </div>
                        <div className="flex items-center gap-1">
                          {account.connected && (
                            <div className="flex items-center gap-1 text-xs text-green-600 dark:text-green-400">
                              <Check className="w-3 h-3" />
                              <span>Connected</span>
                            </div>
                          )}
                          <button
                            onClick={() => handleRemovePlatform(account.id)}
                            className="opacity-0 group-hover:opacity-100 transition-opacity p-1 hover:bg-muted rounded"
                            title="Remove from sidebar"
                          >
                            <Trash2 className="w-3 h-3 text-muted-foreground hover:text-destructive" />
                          </button>
                        </div>
                      </div>

                      {account.connected ? (
                        <div className="flex items-center justify-between">
                          <p className="text-xs text-muted-foreground">
                            {account.username}
                          </p>
                          <Button
                            variant="ghost"
                            className="h-7 text-xs px-2"
                            onClick={() => console.log('Manage', account.id)}
                          >
                            Manage
                          </Button>
                        </div>
                      ) : (
                        <Button
                          variant="neutral"
                          size="sm"
                          className="w-full h-8 text-xs gap-1"
                          onClick={() => handleConnectPlatform(account.id)}
                        >
                          <Plus className="w-3 h-3" />
                          Connect
                        </Button>
                      )}
                    </div>
                  )}
                </div>
              );
            })}
          </div>

          {/* Add More Platforms Button & Dialog */}
          {!isCollapsed && (
            <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              <DialogTrigger asChild>
                <Button
                  variant="ghost"
                  className="w-full mt-3 text-xs gap-2 text-muted-foreground hover:text-foreground"
                >
                  <Plus className="w-4 h-4" />
                  Add More Platforms
                </Button>
              </DialogTrigger>
              <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
                <DialogHeader>
                  <DialogTitle>Add Social Media Platforms</DialogTitle>
                  <DialogDescription>
                    Select platforms to add to your sidebar. You can connect and manage them later.
                  </DialogDescription>
                </DialogHeader>

                <div className="space-y-6 py-4">
                  {/* Social Platforms */}
                  <div>
                    <h4 className="text-sm font-semibold mb-3 text-foreground">Social Media</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {platformsByCategory.social.map((platform) => {
                        const Icon = platform.icon;
                        const account = socialAccounts.find(a => a.id === platform.id);
                        const isVisible = account?.isVisible || false;

                        return (
                          <button
                            key={platform.id}
                            onClick={() => handleTogglePlatformVisibility(platform.id)}
                            className={cn(
                              "flex items-center gap-3 p-3 rounded-lg border transition-all",
                              isVisible
                                ? "bg-primary/10 border-primary"
                                : "bg-muted/30 border-border hover:border-primary/50"
                            )}
                          >
                            <Icon className="w-5 h-5" style={{ color: platform.color }} />
                            <span className="text-sm font-medium flex-1 text-left">
                              {platform.platform}
                            </span>
                            {isVisible && <Check className="w-4 h-4 text-primary" />}
                          </button>
                        );
                      })}
                    </div>
                  </div>

                  {/* Professional Platforms */}
                  <div>
                    <h4 className="text-sm font-semibold mb-3 text-foreground">Professional</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {platformsByCategory.professional.map((platform) => {
                        const Icon = platform.icon;
                        const account = socialAccounts.find(a => a.id === platform.id);
                        const isVisible = account?.isVisible || false;

                        return (
                          <button
                            key={platform.id}
                            onClick={() => handleTogglePlatformVisibility(platform.id)}
                            className={cn(
                              "flex items-center gap-3 p-3 rounded-lg border transition-all",
                              isVisible
                                ? "bg-primary/10 border-primary"
                                : "bg-muted/30 border-border hover:border-primary/50"
                            )}
                          >
                            <Icon className="w-5 h-5" style={{ color: platform.color }} />
                            <span className="text-sm font-medium flex-1 text-left">
                              {platform.platform}
                            </span>
                            {isVisible && <Check className="w-4 h-4 text-primary" />}
                          </button>
                        );
                      })}
                    </div>
                  </div>

                  {/* Media Platforms */}
                  <div>
                    <h4 className="text-sm font-semibold mb-3 text-foreground">Media & Content</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {platformsByCategory.media.map((platform) => {
                        const Icon = platform.icon;
                        const account = socialAccounts.find(a => a.id === platform.id);
                        const isVisible = account?.isVisible || false;

                        return (
                          <button
                            key={platform.id}
                            onClick={() => handleTogglePlatformVisibility(platform.id)}
                            className={cn(
                              "flex items-center gap-3 p-3 rounded-lg border transition-all",
                              isVisible
                                ? "bg-primary/10 border-primary"
                                : "bg-muted/30 border-border hover:border-primary/50"
                            )}
                          >
                            <Icon className="w-5 h-5" style={{ color: platform.color }} />
                            <span className="text-sm font-medium flex-1 text-left">
                              {platform.platform}
                            </span>
                            {isVisible && <Check className="w-4 h-4 text-primary" />}
                          </button>
                        );
                      })}
                    </div>
                  </div>

                  {/* Messaging Platforms */}
                  <div>
                    <h4 className="text-sm font-semibold mb-3 text-foreground">Messaging</h4>
                    <div className="grid grid-cols-2 gap-2">
                      {platformsByCategory.messaging.map((platform) => {
                        const Icon = platform.icon;
                        const account = socialAccounts.find(a => a.id === platform.id);
                        const isVisible = account?.isVisible || false;

                        return (
                          <button
                            key={platform.id}
                            onClick={() => handleTogglePlatformVisibility(platform.id)}
                            className={cn(
                              "flex items-center gap-3 p-3 rounded-lg border transition-all",
                              isVisible
                                ? "bg-primary/10 border-primary"
                                : "bg-muted/30 border-border hover:border-primary/50"
                            )}
                          >
                            <Icon className="w-5 h-5" style={{ color: platform.color }} />
                            <span className="text-sm font-medium flex-1 text-left">
                              {platform.platform}
                            </span>
                            {isVisible && <Check className="w-4 h-4 text-primary" />}
                          </button>
                        );
                      })}
                    </div>
                  </div>
                </div>

                <div className="flex justify-end gap-2 pt-4 border-t">
                  <Button
                    variant="ghost"
                    onClick={() => setIsDialogOpen(false)}
                  >
                    Close
                  </Button>
                </div>
              </DialogContent>
            </Dialog>
          )}
        </div>
      </div>
    </aside>
  );
};
