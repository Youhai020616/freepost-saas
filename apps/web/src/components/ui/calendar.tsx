...
// Default class names defined outside component to prevent recreation on every render
...
// Chevron component defined outside to prevent recreation on every render
...
const ChevronComponent = (props: ChevronProps) => {
  const { orientation, className, size = 16 } = props;
  if (orientation === "left") {
    return <ChevronLeft size={size} strokeWidth={2} className={className} aria-hidden="true" />;
  }
  return <ChevronRight size={size} strokeWidth={2} className={className} aria-hidden="true" />;
};

// Default components defined outside to prevent recreation
...
function Calendar({
  ...
  // Memoize merged class names to prevent unnecessary recalculations
